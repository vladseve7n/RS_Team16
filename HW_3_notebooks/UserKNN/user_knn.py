...
class UserKnn:
    ...
    def fit(self, train: pd.DataFrame, show_progress: bool = True):
        ...
        self.user_knn.fit(self.weights_matrix, show_progress=show_progress)
        self.is_fitted = True
...
class UserKnnBM25:
    ...
    def fit(self, train: pd.DataFrame, show_progress: bool = True):
        self.get_mappings(train)
        self.weights_matrix = self.get_matrix(train)
        self.model.fit(self.weights_matrix, show_progress=show_progress)
        self.is_fitted = True
    ...
    def predict_for_one_user(self, user_id: int, N_recs: int = 10) -> List[int]:
        ...
        result = recs[recs['rank'] <= N_recs]['item_id'].tolist()[:N_recs]
